...
int main(int argc, char** argv)
{
  ...
  unsigned char *data_packet = (unsigned char*)malloc(0);

	/* RESTO DO PROTOCOLO (INFO FRAMES ETC.) */
   unsigned char *final_data_packet = (unsigned char*)malloc(0);
	 while (indice < fileSize)
	{
		//cut the message in the bytesForEachPacket
		cutMessage(message, &indice, &sizeDP, fileSize, data_packet);
		printf("Sent packet nr %d\n", numTotalPackets);

		//packet header
		int headerSize = sizeDP;
		packetHeader(data_packet, final_data_packet, &headerSize, fileSize);
...
int llwrite(int fd, char * buffer, int length) {
  ...
  printf("366 \n");
	...
	stuffingBCC2(BCC2Stuffed, BCC2, &sizeBCC2);
...
void stuffingBCC2(unsigned char *BCC2Stuffed, unsigned char BCC2, int *sizeBCC2) {

  unsigned char *otherBCC2Stuffed = (unsigned char *)malloc(2 * sizeof(unsigned char));

  if (BCC2 == FLAG) {
    otherBCC2Stuffed[0] = ESCAPE;
    otherBCC2Stuffed[1] = ESCAPE_FLAG;
    (*sizeBCC2)++;
    memcpy(BCC2Stuffed, otherBCC2Stuffed, (2 * sizeof(unsigned char)) );
  }
  else {
    if (BCC2 == ESCAPE) {
      otherBCC2Stuffed[0] = ESCAPE;
      otherBCC2Stuffed[1] = ESCAPE_ESCAPE;
      (*sizeBCC2)++;
      memcpy(BCC2Stuffed, otherBCC2Stuffed, (2 * sizeof(unsigned char)) );
    }
  }

  free(otherBCC2Stuffed);
}
...
void packetHeader(unsigned char *message, char *final_data_packet, int *sizeDP, off_t fileSize)
{
	*sizeDP += 4;
	...
	numTotalPackets++;
  printf("619 \n");
	memcpy(final_data_packet, finalPacket, *sizeDP);
  printf("621 \n");
  free(finalPacket);
  printf("623 \n");
}

void cutMessage(unsigned char *message, off_t *indice, int *sizeDP, off_t fileSize, unsigned char *data_packet)
{
  long int pos = *indice;
	if((pos + *sizeDP) > fileSize)
	{
		*sizeDP = fileSize - pos;
	}

	unsigned char * packet = (unsigned char *)malloc(*sizeDP);
	for(int i = 0; i < *sizeDP; i++, pos++)
	{
		packet[i] = message[pos];
	}

  *indice = pos;
	memcpy(data_packet, packet, *sizeDP);
  free(packet);
}
